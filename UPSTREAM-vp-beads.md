...
- **bd v1.1.0 schema migration panics on drifted TEXT storage, write-locking the database** (2026-07-07) \[blocking\] — After Homebrew auto-upgraded `beads` 1.0.5 → 1.1.0, all bd **writes** fail (`bd create`, `bd config unset`, `bd migrate`) while reads still work. bd v1.1.0's remote-migrate gate (#4259) first refuses the 4 pending v49→v53 schema migrations on a remote-backed DB; but the migration itself then **panics** — `rekey aux row ids: events: invalid hash length: 19` — on drifted adaptive-TEXT storage, so removing the remote lifts the gate yet the next write re-triggers migrate-on-open and crashes again. Root cause: bd's embedded Dolt pin predates the dolthub/dolt#11126 fix (dolt#11131 / #11205 — adaptive out-of-line TEXT written by an earlier version is unreadable; rows "not repairable via SQL"). Blocks all issue creation/updates for this project. Upstream: <https://github.com/gastownhall/beads/issues/4380> (exact events-table crash, fix "skip-and-warn per table" filed 2026-07-07; see also gastownhall/beads#4590 and #4353).
  Severity: blocking · Ownership: upstream · Workaround: partial — reads still work; unblock either by downgrading to `beads` 1.0.5 (no v49→v53 migration) or by reinitializing a fresh v53 DB and reimporting the JSONL export. Chose to WAIT for the upstream fix; full backup preserved at `.beads/backup-pre-local-2026-07-07.jsonl`.
