 ...
 * link text is not recoverable here. `scripts/read-link-context.mjs` supplies it
 * later, and it is required before anything is rewritten.
 *
 ...
 * handoff, so `scripts/check-link-resolution.mjs` pins these strings the same
 * way `lib/staleness-contract.mjs` pins the drift buckets.
 *
...
export function classifyEdge (edge, index) {
  if (!['in-relations', 'outside-relations', 'unresolvable'].includes(edge.sectionProvenance)) {
    throw new TypeError(
      ...
      'annotate edges with scripts/read-link-context.mjs before classifying. Refusing to default it, ' +
