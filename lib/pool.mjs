 ...
 *
 * Extracted after `lib/ndjson.mjs`'s header predicted these copies would
 * diverge and they did not — see that file for the forecast and why it was
 * wrong. Three copies existed: two named `pool()`, and one inlined in
 * `downloads-batch.mjs` that a grep for `pool` does not find.
 *
