...
export const CANONICAL_FOURTH_WALL_RULES = [
  ...
  // "Graph Coverage" gets a SAME-LINE negative lookahead `(?! \w)` so
  // "## Graph Coverage Algorithms" (a legitimate subject heading) does not fire,
  // while a bare "## Graph Coverage" heading followed by body still does. A
  // `\s`-based lookahead would cross the newline into the body paragraph and miss
  // every real heading (the body's first word satisfies `\s+\w`).
