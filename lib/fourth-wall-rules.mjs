...
export const CANONICAL_FOURTH_WALL_RULES = [
  ...
  // "Graph Coverage" gets a negative lookahead so "## Graph Coverage Algorithms"
  // (a legitimate subject heading) does not fire; bare "## Graph Coverage" still does.
  { id: 'fw-self-ref-section', name: 'No self-referential sections', deterministic: true, detect: /^#{1,6}\s.*(Connection to the Knowledge Graph|Graph Coverage(?!\s+\w)|Significance to the Graph|Why This Note Exists)/im, summary: 'No "Connection to the Knowledge Graph" / "Graph Coverage" headings.' },
