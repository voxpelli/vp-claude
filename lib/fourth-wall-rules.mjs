 ...
 * The invariant is now both type-checked (tsconfig + `check:tsc` are wired) and
 * asserted at runtime by the self-test (belt-and-suspenders — the runtime check
 * also covers the JS callers that don't run through tsc).
 *
 * @typedef {DeterministicRule | JudgmentRule} FourthWallRule
 */

/**
 * The stable set of rule ids, derived from the registry so it can never drift
 * from the actual array.
 *
 * @typedef {typeof CANONICAL_FOURTH_WALL_RULES[number]['id']} FourthWallRuleId
 */

export const CANONICAL_FOURTH_WALL_RULES = /** @satisfies {FourthWallRule[]} */ (/** @type {const} */ ([
...
export function detectFourthWallViolations (text) {
  ...
  for (const rule of CANONICAL_FOURTH_WALL_RULES) {
    // The `deterministic` discriminant alone narrows `rule` to DeterministicRule,
    // so `rule.detect` is a typed RegExp on the surviving path — no `!rule.detect`
    // guard needed. The self-test still asserts the invariant holds at runtime.
    if (!rule.deterministic) continue
...
export function checkDetectionColumnParity (checklistContent) {
  ...
  while ((m = rowRe.exec(checklistContent)) !== null) {
    const [, id, found] = m
    // The regex has two non-optional capture groups, so a successful match
    // always yields both — narrow for tsc (JSDoc isn't a runtime guarantee).
    if (id === undefined || found === undefined) continue
    seen.add(id)
    if (!byId.has(id)) {
