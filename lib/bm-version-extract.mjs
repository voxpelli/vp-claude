 ...
 * the same signal every real `/intel` npm note carries), where
 ...
 *   3. `[version]` / `[version-range]` observation (the first such line that
 *      yields a version token, not merely the first line so labelled)
...
function parseForCompare (v) {
  ...
  return [Number.parseInt(major, 10), Number.parseInt(minor, 10), Number.parseInt(m[3] ?? '0', 10)]
}
...
function patternInlineHeaderPipe (content) {
  ...
  const value = m?.[1]
  return value ? { value, isRange: false } : null
}
...
function patternTableRow (content) {
  for (const line of content.split('\n')) {
    ...
    const [label, valueCell] = cells
    if (label === undefined || valueCell === undefined) continue
    if (label.toLowerCase() !== 'version') continue
    const valueMatch = /v?(\d[\w.+-]*)/.exec(valueCell)
    const value = valueMatch?.[1]
    if (value) return { value, isRange: false }
  }
  return null
}
 ...
 *
 * Scans EVERY matching observation line and returns the first one that yields
 * a version token — not merely the first line that carries the label. A real
 * note routinely has several `[version]` observations, and a narrative one
 * ("Homebrew has not yet bumped …") sorts above the machine-readable slot as
 * often as not; bailing on the first unparseable line made the whole note
 * invisible to `--stale`. This mirrors Pattern 2, which has always
 * loop-and-continued over candidate table rows.
 *
 * @param {string} content
 * @returns {ExtractedToken | null}
 */
function patternObservation (content) {
  for (const m of content.matchAll(/^-\s*\[(version|version-range)\](.+)$/gm)) {
    const label = m[1]
    const rawValue = m[2]
    if (label === undefined || rawValue === undefined) continue
    const isRange = label === 'version-range'
    const raw = rawValue.trim()
    const source = isRange ? stripLeadingRangeOperator(raw) : raw
    const tokenMatch = /^v?(\d[\w.+-]*)/.exec(source)
    if (!tokenMatch || tokenMatch[1] === undefined) continue
    return { value: tokenMatch[1], isRange }
  }
  return null
}
...
function patternFrontmatterVersion (content) {
  ...
  return { value: m[1], isRange: false }
}
...
function patternReleaseHighlights (content) {
  ...
  /** @type {string[]} */
  const tokens = []
  for (const m of block.matchAll(/\*\*v?(\d+\.\d+\.\d[\w.-]*)\*\*/g)) {
...
function patternProseFallback (content) {
  ...
  if (bulletMatch && bulletMatch[1] !== undefined) return { value: bulletMatch[1], isRange: false }
  const currentMatch = /^\s*Current:\s*v?(\d[\w.+-]*)/m.exec(content)
  if (!currentMatch || currentMatch[1] === undefined) return null
  return { value: currentMatch[1], isRange: false }
}
 ...
 * signal every real `/intel`-written npm note carries
 ...
 * @param {string} [_noteTitle] - the note's title; not used by the current
 ...
 *   `null` version.
 */
export function extractBmVersion (noteContent, _noteTitle) {
