...
/**
 * @param {string} url
 * @returns {string|null} owner/repo parsed from a GitHub git URL
 */
...
export function resolveInstalledPlugins (installedPluginsContent, knownMarketplacesContent, marketplaceContents) {
  const installed = JSON.parse(installedPluginsContent)
  const known = JSON.parse(knownMarketplacesContent)
  ...
  for (const [key, records] of Object.entries(installed.plugins ?? {})) {
    ...
    const installedAt = first && typeof first.installedAt === 'string' ? first.installedAt : null

    let entry
    const mpContent = marketplaceContents.get(marketplace)
    if (mpContent) {
      try {
        const mp = JSON.parse(mpContent)
        entry = (mp.plugins ?? []).find((/** @type {any} */ p) => p.name === name)
    ...
    if (typeof src === 'string') {
      ...
      if (typeof repo === 'string' && /^[^/\s]+\/[^/\s]+$/.test(repo)) identifier = pluginIdentifier(repo, name)
    } else if (src && src.source === 'github' && typeof src.repo === 'string') {
      identifier = `plugin:${src.repo}` // dedicated repo — no disambiguator needed
    } else if (src && src.source === 'git-subdir' && typeof src.url === 'string') {
...
export function resolveInstalledSkills (skillLockContent) {
  const lock = JSON.parse(skillLockContent)
  ...
  for (const [dir, info] of Object.entries(lock.skills ?? {})) {
    const source = /** @type {any} */ (info)?.source
    const resolved = typeof source === 'string' && source.length > 0
    // A dir absent from any source groups under a unique key so it stays name-only.
    const groupKey = resolved ? source : `\0${dir}`
    const g = bySource.get(groupKey) ?? { members: [], installedAt: null, resolved }
    g.members.push(dir)
    const ts = /** @type {any} */ (info)?.installedAt
    if (typeof ts === 'string' && (!g.installedAt || ts > g.installedAt)) g.installedAt = ts
