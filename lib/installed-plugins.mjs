...
//
// Identifier scheme (round-trips with `/intel plugin:`/`skill:` and the BM note
// title convention `:`/`/`/`#` -> `-`):
...
/**
 * Dependency-free key-is-string guard, kept local so this module stays
 * importable in a Claude marketplace install (whole-tree copy, no `npm install`,
 * gitignored node_modules — `/knowledge-gaps --global` shells
 * `node scripts/list-installed-plugins.mjs` into it). Replaces typed-utils'
 * `isKeyWithType(x, k, 'string')` — the only shape this module used.
 *
 * @template {string} K
 * @param {unknown} input
 * @param {K} key
 * @returns {input is Record<K, string>}
 */
export function hasStringKey (input, key) {
  return typeof input === 'object' && input !== null && key in input &&
    typeof /** @type {Record<string, unknown>} */ (input)[key] === 'string'
}
...
export function resolveInstalledPlugins (installedPluginsContent, knownMarketplacesContent, marketplaceContents) {
  ...
  for (const [key, records] of Object.entries(installed.plugins ?? {})) {
    ...
    const installedAt = hasStringKey(first, 'installedAt') ? first.installedAt : null
    ...
    if (typeof src === 'string') {
      ...
      if (hasStringKey(marketplaceSource, 'repo') && /^[^/\s]+\/[^/\s]+$/.test(marketplaceSource.repo)) {
        identifier = pluginIdentifier(marketplaceSource.repo, name)
      }
    } else if (src && src.source === 'github' && hasStringKey(src, 'repo')) {
      identifier = `plugin:${src.repo}` // dedicated repo — no disambiguator needed
    } else if (src?.source === 'git-subdir' && hasStringKey(src, 'url')) {
...
export function resolveInstalledSkills (skillLockContent) {
  ...
  for (const [dir, info] of Object.entries(lock.skills ?? {})) {
    const source = hasStringKey(info, 'source') ? info.source : undefined
    ...
    g.members.push(dir)
    if (hasStringKey(info, 'installedAt') && (!g.installedAt || info.installedAt > g.installedAt)) {
