...
// it uses an entirely different heading scheme ("## Provenance", "## Open
// items") rather than the standard vocabulary — and is excluded from the
// membership check entirely via the allowlist below, not partially checked.
//
// `Resolved` was removed from the vocabulary on 2026-08-28. The owning skill
// (vp-beads upstream-tracker) resolves entries by DELETING them, since git
// history is the record; the `### Resolved` subsection it describes lives in
// the Basic Memory note, not here. Listing it as canonical invited an archive
// section that contradicted that — the one file that had it kept it empty for
// months. A `## Resolved` heading now fails the check, which is the intent.
//
