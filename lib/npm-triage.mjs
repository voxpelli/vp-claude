 ...
 * vacuous — this repo keeps shipping that mistake — so the two sides of
 ...
 *
 * @typedef RegistryRow
 * @property {string} id - the join key. Declared here now: `readNdjson` makes a
 *   string `id` a hard runtime precondition of every row it returns, and only
 *   `ScanRow` said so. `registry-shard.mjs` had already patched the gap locally
 *   by returning `NdjsonRow & RegistryRow` — noticing it for one of the three
 *   and fixing it at the call site rather than in the type
 ...
 * @property {string} id - see `RegistryRow.id`
 * @property {number | null} weeklyDownloads - three states, not four. This was
 *   OPTIONAL *and* nullable, which declares an absent-vs-null distinction that
 *   no producer emits (every path sets it explicitly) and no consumer reads
 *   (`typeof d?.weeklyDownloads === 'number' ? … : null` collapses them). It
 *   also contradicted `lib/npm-downloads.mjs`'s typedef for the identical
 *   record, two files away, which has it required
