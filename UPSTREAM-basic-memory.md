...
**Status:** Open — workaround in place, but the API behavior is surprising and undocumented. `edit_note` silently ACCEPTS and discards the unused `section` argument on `append`/`prepend` (only `replace_section`/`insert_*_section` consume it); a validation error would surface the mistake instead of silently misplacing content (minor UX filing). **0.22.1 update:** `edit_note` now offers first-class `insert_before_section` / `insert_after_section` operations that place content adjacent to a heading without consuming the section body — these supersede the `find_replace`-on-last-line workaround for the common "add right after a heading" case (appending to the END of an existing section's content still needs `find_replace`).
...
**Measured impact (2026-06-16):** a graph-wide decomposition of the 1,286 unresolved relations found **116 edges (~9%)** whose target exists under a longer *descriptive* title — `[[Pelle Wessman]]` → `Pelle Wessman - IndieWeb Builder and Open Source Maintainer`, `[[Conservative Change Philosophy]]` → `…- If It Ain't Broken Don't Fix It`, `[[Fastify]]` → `npm-fastify`, etc. Alias- or prefix-resolution in LinkResolver would clear these in bulk; today each requires a manual per-link retarget.
**Rename is not a viable workaround** (verified 2026-06-16): renaming a descriptively-titled note to its bare name *inverts* the problem. The `Pelle Wessman` person note has **97 inbound edges on the full descriptive title vs 4 on the bare name** (plus a 3rd drifted variant `Pelle Wessman - Open Source Developer`). A rename keeps the 97 resolved only via their stale `to_id` FK until each referencing note is next edited, then orphans them — a 24:1 bad trade. Per-link retarget or an upstream alias/fuzzy resolver are the only safe fixes.
**Wider context — this is not BM-specific, and BM is the outlier:** the underlying tension (descriptive note titles for human scanning vs `[[bare name]]` links for natural authoring) is universal to wiki-link PKM tools, and the ecosystem has a de-facto-standard reconciliation: **Obsidian resolves `[[X]]` against `aliases:` frontmatter, and Logseq against `alias::`** — both first-class. BM otherwise tracks the Obsidian-flavored convention (frontmatter, `## Observations`/`## Relations`, wiki-links) but is the **lone holdout** on alias resolution. Honoring `aliases` frontmatter in LinkResolver would (a) close the 116-edge gap here, (b) restore Obsidian/Logseq interop, and (c) let descriptive titles and natural-name links coexist — which today they cannot. The mechanism is well-precedented (two major tools ship it), so this is a low-novelty, high-leverage filing rather than a research proposal. NB: any committable-index layer that claims Obsidian compatibility (e.g. `.md-wiki-vec`) inherits the same requirement — alias-aware resolution is table stakes for the convention, not a BM extra.
...
**Impact:** Medium — an observation line of the form `- [category] … [text](url) … (trailing parenthetical)` is silently dropped from the parsed observation index. `schema_validate` still reports a clean pass, so the loss is invisible without a before/after observation count. Reproduced on an `engineering` note: a `## Key Sources` block of 8 `[source]` lines parsed as 7 — the one line carrying an inline markdown link followed by a trailing `(link post …)` aside vanished, while its 7 siblings parsed. **Root cause confirmed (2026-07-15, installed 0.22.1 source):** `is_observation`'s pure-markdown-link exclusion regex (`^\[.*?\]\(.*?\)$`) is meant only to skip a line that IS just a bare markdown link, but its non-greedy `.*?` backtracks across intermediate brackets, so it ALSO matches `[category] …[text](url)` whenever the whole content starts with `[` and ends with `)`. The line then never reaches `parse_observation` and is dropped. Scope is therefore BROADER than a *trailing* parenthetical: any `[category]` observation whose content contains an embedded `[text](url)` and ends in `)` is silently dropped — a separate trailing `(context)` is not required; the link's own closing `)` suffices. Minimal fix: anchor the bracket char-classes (e.g. `^\[[^\]]*\]\([^)]*\)$`) so it matches only a line that is purely one bare markdown link. Confirmed fileable bug (silent data loss).
...
**Status:** Open locally, **unreported upstream (verified 2026-06-10 — no matching issue exists)**. Nearest neighbors share the likely async-indexing root: [#763](https://github.com/basicmachines-co/basic-memory/issues/763) (write tools return inside the indexing window, ruled by-design) and [#940](https://github.com/basicmachines-co/basic-memory/issues/940) (relation batch-indexing races, open). [#931](https://github.com/basicmachines-co/basic-memory/pull/931) (merged 2026-06-10) is the opposite signature — persistent silent drop via permalink-dedup, not transient inflation — but proves the index layer dedups observations by permalink, the mechanism a double-parse would interact with. High-value filing citing all three. **0.22.1 status update (2026-07-15):** a source-grounded read of installed 0.22.1 found NO static mechanism that would transiently inflate/deflate the echoed counts — the `edit_note` response counts come from the synchronous PATCH result, and the only async path is background FTS/embedding indexing (which affects *search* hits, not the echoed counts). So this is either fixed since the 0.21.x reproductions above, or driven by a runtime-async effect not visible in a static read. **Downgraded to UNCONFIRMED on 0.22.1 pending a live re-test** on a `###`-subsectioned note — but do NOT delete: the 0.21.x reproductions (brew-glab 5→1, cask-1password ×3) were real observations.
...
### `[[wiki-links]]` in prose/observation context are silently extracted as spurious relations

**Discovered:** 2026-06-16 (recent-stuff gap-closure pass — `Flattr` cleanup + new OAuth 2.0 / OAuth 2.1 standard notes)
**Impact:** Medium — BM's whole-note re-parse extracts *every* `[[wiki-link]]`, not only those under `## Relations`. A `[[link]]` in body prose or an observation line therefore mints a relation. When prose *precedes* the `[[` on the same line, that prose is absorbed into the `relation_type` field (a corrupted relation); when little/no prose precedes it, a spurious `links_to` edge appears (post-[#824](https://github.com/basicmachines-co/basic-memory/issues/824)). Either way the file looks correct and `schema_validate` passes, so the bad edge is invisible without a relation-index re-query. Hit 3× this session: the `Flattr` note's `## People` bullet (`…payment routing. See [[Flattr and relspider…]]`) created a relation whose `relation_type` was the entire sentence; two stray `links_to` edges appeared in the new OAuth 2.0 / OAuth 2.1 notes from prose wiki-links (both cleaned).
**Workaround:** full — keep `[[ ]]` strictly inside `## Relations`; reference other notes from prose in plain text. Removing the prose `[[` and re-parsing drops the bad relation (verified on `Flattr`: the corrupted relation cleared on re-parse, leaving only the clean `context_for` edge with a fresh `relation_id`).
**Status:** Open, unreported upstream. This is the **silent, under-200-char sibling** of the "`edit_note` re-parse triggers validation errors from unrelated notes" entry above — that one *errors* (relation_type >200 chars → `MaxLen(200)`, regression-guarded by [PR #770](https://github.com/basicmachines-co/basic-memory/pull/770)); this one *succeeds* and silently pollutes the relation index. A real fix would scope relation extraction to the `## Relations` section (or to genuine observation/relation contexts), not arbitrary prose. High-value filing — pairs with #770/#824.
Severity: degraded · Ownership: upstream · Workaround: full — usage convention (`[[ ]]` only under `## Relations`), but the silent/invisible failure mode makes it easy to violate.

---

### Forward-reference relations are never re-resolved after their target is created — only re-writing the *referencing* note fixes them

**Discovered:** 2026-06-16 (graph-wide "most missing edges" decomposition + a targeted resolution experiment)
**Impact:** Medium — a decomposition of the 1,286 unresolved relations found **83 (~6.5%) are "phantom": the target note already exists** (exact title, same project) yet the edge stays `to_id NULL`. `bm status` reports "main: No changes" (files↔db fully in sync), so this is not a sync lag — BM never re-resolves a forward-reference once its target appears. Live, not historical: `npm-prettier`, `npm-ajv-cli`, and `OAuth 2.1` — all created earlier in this same session — each already carry 2–3 phantom inbound edges from notes that were not themselves re-written. No CLI re-resolves relations (`bm reindex` rebuilds FTS + vector embeddings only; `status`/`doctor` are read-only).
**Experiment (2026-06-16, `npm-prettier` = entity 1628, 3 phantom inbound edges):**
- Editing the **target** (`npm-prettier`) via `edit_note` → the 3 inbound phantom edges stayed `NULL`. **Ingress resolution does NOT re-fire on a target edit.**
- Editing a **referencing** note (`brew-biome`, which links `competes_with [[npm-prettier]]`) → that egress edge resolved to 1628 immediately and persisted through a later edit. **Egress resolution re-fires only on the source note's own write.**
**Workaround:** partial — to clear a phantom edge, re-write the **referencing** note (any real `edit_note` re-fires its egress pass); editing the target is useless. Cost is **O(referencing notes)**, not O(targets) — a hub with N phantom inbound edges needs N distinct source-note edits.
**Status:** Open, unreported upstream. Same async/batch relation-indexing family as [#940](https://github.com/basicmachines-co/basic-memory/issues/940) and [#763](https://github.com/basicmachines-co/basic-memory/issues/763), but distinct: *persistent* (survives a clean `bm status`) and *directional* (only egress re-resolves). A sync-time "resolve dangling relations whose `to_name` now matches an entity" pass — or an ingress-resolution step on entity create/update — would fix it. **Repro:** create A linking `[[B]]`; later create B titled exactly `B`; `A→B` stays `to_id NULL` until A (not B) is re-written.
Severity: degraded · Ownership: upstream · Workaround: partial — re-write each referencing note.

---

### `created_at` is overwritten to edit-time on every markdown-note write (no `is_new_entity` gate)

**Discovered:** 2026-07-15 (source-grounded deep-dive of installed BM 0.22.1; empirically reproduced via an MCP create→edit→re-read probe that saw `created_at` advance from the create time to the edit time)
**Impact:** Medium — a markdown note's `created_at` does not track authorship; it is re-derived on every sync and advances to (effectively) the last edit time, so a months-old note edited today reports "created today." This makes `created_at` unusable for age/provenance reasoning ("which notes predate this session") and silently undermines any staleness logic that trusts it. Mechanism (paraphrased, 0.22.1): the markdown sync path (`sync_service.sync_one_markdown_file` + `entity_service._apply_markdown_entity_fields`) unconditionally sets `created_at` from the parsed `created` value, which `entity_parser.parse_file_content` derives from `file_service.get_file_metadata`'s `os.stat().st_ctime` — the POSIX inode *change*-time, not a birth time. Because `file_utils.write_file_atomic` writes a temp file then `Path.replace()`-renames it over the target, every `edit_note`/`write_note` bumps `st_ctime`, which is copied straight into `created_at`. ASYMMETRIC: the regular/attachment path (`batch_indexer._upsert_regular_file`) gates this with `include_created_at=is_new_entity`, preserving `created_at` for existing attachments — the markdown path has no equivalent gate.
**Workaround:** full — use git history of the note file for true authorship/creation dates; never trust BM `created_at` for provenance.
**Status:** Open, unreported upstream. File as a *question*, not an assumed bug — the markdown/attachment asymmetry may be an oversight (the newer markdown pipeline never got the older path's gate) or intentional (treating `created` as content-parse time). Verified against installed 0.22.1 (Homebrew).
Severity: degraded · Ownership: upstream · Workaround: full — git for authorship dates.

---

### No local relation-repair pass — `bm reindex` rebuilds only FTS/embeddings (Feature Request)

**Discovered:** 2026-07-15 (source-grounded deep-dive of installed BM 0.22.1)
