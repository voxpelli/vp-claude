...
2. `read the "schema-evolve" skill` to bring the local file
...
`list_directory` (via the `mcp` proxy) plus the
`search_notes` workaround, but flag the result as
...
3. For packages with 3+ imports and no dedicated note, invoke the
   `intel` skill with the appropriate prefix:
   ```
   /skill:intel crate:serde
   /skill:intel pypi:requests
   /skill:intel go:github.com/gin-gonic/gin
   /skill:intel composer:laravel/framework
   /skill:intel gem:rails
   /skill:intel fastify  # npm (no prefix)
   ```

4. **Detect tool manifests** — check for tool manifest files:
   - `read("./Brewfile")` → brew formulae, casks, and vscode extensions
   - `find . -name ".github/workflows/*.yml"` / `find . -name ".github/workflows/*.yaml"` → GitHub Actions
   - `read("./Dockerfile")`, `find . -name "*.dockerfile"`, `find . -name "Dockerfile.*"` → Docker images
   - `read("./.vscode/extensions.json")` → VSCode extensions

5. For undocumented tools from detected manifests, invoke the `intel`
   skill with the appropriate prefix:
   ```
   /skill:intel brew:ripgrep
   /skill:intel cask:warp
   /skill:intel action:actions/checkout
   /skill:intel docker:grafana/grafana
   /skill:intel vscode:esbenp.prettier-vscode
   /skill:intel gh:meiji163/gh-notify
   ```
...
unlike Section 3 (which delegates directly to a research skill by reading
it for *new*, undocumented packages), Section 3b does NOT invoke a research
...
2. Invoking `/intel` by reading the skill from
...
version-only bump because it believed "reading a skill only loads
...
- **`Drifted >30d`** → routine queue tier. After the pre-enqueue re-read
  ...
  **Refresh Queue → Routine** lane of the report (format below), citing
  the prefix's refresh command from the table above. Do NOT read the
  skill and do NOT `edit_note` for these targets — this bucket is
