...
The 15 skills, with the behaviour the `CLAUDE.md` index summarises:
...
- **deep-intel** — The research front-door for the six knowledge-type schemas (`service`/`concept`/`standard`/`milestone`/`project`/`engineering`) that the package/tool/people skills don't cover. Replicates the built-in `/deep-research` engine as its own dynamic **Workflow** (type-targeted angle decomposition → parallel search → streaming fetch+extract → graph-aware draft+synthesis) but returns a proposed Basic Memory note rather than a report. Two human gates live in the foreground skill: a **pre-spend** gate before launching the Workflow, and a **pre-write** gate on the completion turn (the Workflow returns a structured proposed-note object and never writes BM itself). Detects and redirects package/tool/person subjects to the sibling skills (print-and-exit). Observation categories + relation verbs are derived from `schemas/<type>.md` at synthesis time (no note templates ship); `references/synthesis-profiles.md` carries per-type body-shape guidance and `references/research-workflow.md` carries the Workflow script. **Explicit `/command` only (`disable-model-invocation: true`)** — it is expensive and shares trigger-space with the sibling intel skills. Output is labelled "multi-source cross-checked", never "fact-checked". The verification topology runs over the persisted set only (the draft note's observations, stake-tiered): HIGH-stakes claims get an adversarial (sources-withheld) + non-adversarial pair with a 3rd-judge tie-break, MEDIUM/LOW get batched pairs, and a macro pair checks framing/omissions; `--quick` skips it. Verdict invariants (a disputed claim is never written as fact, a refuted claim is dropped, a hedged claim carries its hedge) are enforced in the Workflow's rollup code, with the contract documented in `references/verification-topology.md`. User-invocable as `/deep-intel <subject> [--type <t>] [--quick]`.
