...
| Agent-leverage check (optional) | How a coding agent invokes the tool — MCP server tools/flags for MCP-native agents (Claude Code), `--json`/machine-readable CLI for bash agents (pi.dev, CI) — for `brew:`/`cask:` only (dev-tool-adjacent casks only); records an honest limited/none verdict rather than forcing a weak angle |
